/* ══════════════════════════════════════════════════════
   syncPlanoParaBroker — sync de plano pro Firestore de cada broker
   ------------------------------------------------------
...
require('./admin'); // garante que o app default (projeto central) existe
...
exports.syncPlanoParaBroker = onDocumentWritten(
  ...
  async (event) => {
    ...
    const appBroker = getApps().find(a => a.name === nomeApp)
      || initializeApp({ credential: cert(credencial) }, nomeApp);
