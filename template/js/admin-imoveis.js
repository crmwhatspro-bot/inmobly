// ════════════════════════════════════════════════
// ADMIN — CMS de Imóveis
// Fotos são comprimidas no navegador (canvas → WebP/JPEG, máx 900px)
// e salvas como data-URL no Firestore: capa no doc principal,
// galeria na subcoleção imoveis/{id}/fotos. Sem Firebase Storage.
// ════════════════════════════════════════════════
...
// ════════════════════════════════════════════════
const TITULOS_PAINEL = { leads: 'Leads', imoveis: 'Imóveis' };

document.querySelectorAll('.admin-nav__btn').forEach(btn => {
  btn.addEventListener('click', () => {
    document.querySelectorAll('.admin-nav__btn').forEach(b => b.classList.toggle('active', b === btn));
    document.querySelectorAll('.admin-tab-panel').forEach(p =>
      p.classList.toggle('active', p.id === 'tab-' + btn.dataset.tab));
    const titulo = $('admin-page-title');
    if (titulo) titulo.textContent = TITULOS_PAINEL[btn.dataset.tab] || '';
    window.scrollTo({ top: 0 });
    if (btn.dataset.tab === 'imoveis' && !carregou) carregarLista();
  });
});

// carrega a lista assim que o admin autentica
onAuthChange(user => {
  if (user && !carregou) carregarLista();
});
...
listaEl.addEventListener('click', async (e) => {
  ...
  if (btn.dataset.acao === 'editar') {
    ...
    btn.disabled = true;
    try {
      await updateDoc(doc(db, 'imoveis', imv.id), { ativo: imv.ativo === false, updatedAt: serverTimestamp() });
...
async function abrirEditor(imv) {
  ...
  mostrarMsg(null);
  form.reset();
  $('imv-cidade').value = imv?.cidade ?? '{{CITY_DEFAULT}}';
  ...
  if (imv) {
    ...
    try {
      const snap = await getDocs(query(collection(db, 'imoveis', imv.id, 'fotos'), orderBy('ordem')));
...
form.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (id) {
      ...
      const ref = await addDoc(collection(db, 'imoveis'), { ...dados, createdAt: serverTimestamp() });
      id = ref.id;
    }

    // sincroniza a subcoleção de fotos em lote
    const batch = writeBatch(db);
    removidas.forEach(fid => batch.delete(doc(db, 'imoveis', id, 'fotos', fid)));
    fotos.forEach((f, i) => {
      if (f.id) {
        batch.update(doc(db, 'imoveis', id, 'fotos', f.id), { ordem: i });
      } else {
        batch.set(doc(collection(db, 'imoveis', id, 'fotos')), {
...
excluirBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    const snap = await getDocs(collection(db, 'imoveis', editId, 'fotos'));
    const batch = writeBatch(db);
    snap.forEach(d => batch.delete(d.ref));
    await batch.commit();
    await deleteDoc(doc(db, 'imoveis', editId));
